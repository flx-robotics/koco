...
class MovejActionState(EventState):
    '''
    State for using MoveJ action server.
    ...
    def __init__(self, joints=[], speed=0.0, offset=[], config_flags=[], robot_ns=''):
        super(MovejActionState, self).__init__(
    ...
    def on_enter(self, userdata):
        ...
        try:
            ...
            check_inputs = [len(userdata.tf_pose_name), len(userdata.db_pose_name), userdata.target_pose is not None, len(self._joints)]
            check_inputs = map(bool, check_inputs)
            if check_inputs.count(True) != 1:
                Logger.logerr("Exactly one of the joints, tf_pose_name, db_pose_name and target_pose should be passed in and not 0 or more than 1!")
                self._error = True
                return
            elif len(self._joints) and len(self._offset):
                Logger.logerr("Offset cannot be applied to the joints.")
                self._error = True
                return
            ...
            if len(self._joints):
                goal.target_array = self._joints
            else:
                goal.target_pose = trg_pose

            if len(userdata.db_pose_name):
                goal.target_joint = db_joints
